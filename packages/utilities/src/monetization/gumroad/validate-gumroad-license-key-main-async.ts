...
export async function validateGumroadLicenseKeyMainAsync(options: {
  ...
  return new Promise(function (
    ...
    const __html__ = `<script>const emptyLicense={email:null,licenseKey:null,purchaseTimestamp:null,validationTimestamp:null,variant:null},variantRegex=/\(([^)]+)\)/;function parseVariant(n){if(n==="")return null;const t=n.match(variantRegex);return t===null?null:t[1]}async function main(){async function n(){try{const a=await(await window.fetch("https://api.gumroad.com/v2/licenses/verify",{body:"increment_uses_count=${incrementUseCount}&license_key="+encodeURIComponent("${trimmedLicenseKey}")+"&product_permalink="+encodeURIComponent("${productPermalink}"),headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},method:"POST"})).json(),{purchase:e,success:s}=a;return s===!1||e.chargebacked===!0||e.disputed===!0||e.refunded===!0?{...emptyLicense,result:"INVALID"}:{email:e.email,licenseKey:"${trimmedLicenseKey}",purchaseTimestamp:e.sale_timestamp,result:"VALID",validationTimestamp:"${validationTimestamp}",variant:parseVariant(e.variants)}}catch{return{...emptyLicense,result:"ENDPOINT_DOWN"}}}window.parent.postMessage({pluginMessage:await n()},"*")}main();</script>`
